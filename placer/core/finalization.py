...
for: three of its jobs are impossible locally.
  ...
  1. MEASURE THE RUN'S OWN NULL. The dependency bound that makes a product of
     ...
     `placer/core/blocks.py`), so the raw aggregates travel here and the certified
...
`placer/core/selection.py` -- while their mean is, by linearity of expectation.
"""
...
#: One definition, in `placer/core/policy.py`. The per-locus policy and
...
from placer.core.policy import (  # noqa: E402
...
#: translation unit. Bodies in `placer/core/mathx.py`.
...
#: Re-exported from `placer/core/supports.py`, which `hypotheses.py` shares --
...
def _conformal_feature(chrom: str, pos: int, bp_left: int, bp_right: int, index: int,
    ...
    from placer.core.conformal import ConformalFeatureVector
...
def apply_event_ebh_selection(calls: list[FinalCall], target_fdr: float) -> None:
    ...
    # `placer/core/selection.py` is where these two steps are defined, argued
...
def apply_dependency_penalty_calibration(result: PipelineResult,
    ...
    """
    from placer.core.dependency import estimate_dependency_penalty
...
def apply_sequence_family_commitments(result: PipelineResult) -> None:
    ...
    already decided.
    """
    from placer.core.breakpoints import append_qc_token
...
def apply_sample_local_conformal_selector(result: PipelineResult,
    ...
    """
    from placer.core.conformal import ConformalNullSelector
