...
consensus segmentable -- `placer/core/segmentation.py` finds the junctions by
...
`build_event_consensus` takes the consensus function as an ARGUMENT:
  ...
  * a caller with `pyabpoa` installed passes
    `placer.io.poa.pyabpoa_consensus`;
...
def single_sequence_consensus(sequences: list[str]) -> str:
    ...
    raise ConsensusUnavailable(
        ...
        "pass `placer.io.poa.pyabpoa_consensus` (pip install pyabpoa) "
...
def analyze_clip_insert_concordance(event_evidence: EventReadEvidence,
    ...
    """
    from placer.core.breakpoints import fixed_window_edit_identity
    from placer.core.te_classifier import is_low_complexity_softclip, is_softclip_source
...
def build_local_fragment_component(component, local_records: list[AlignedRead],
    ...
    from placer.alignment import cigar_index
    from placer.core.breakpoints import robust_local_split_insertion_positions
    from placer.core.clustering import INSERTION_CANDIDATE_REQUIRED_MAPQ
