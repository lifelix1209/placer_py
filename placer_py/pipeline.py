...
THE SHAPE OF A RUN:
    ...
    stream the BAM                                    placer_py/io/bam
      -> gate1: is this read worth carrying?           placer_py/io/gate
      -> group into bins                               placer_py/core/bins
    ...
    then, once, over the whole run:
      -> finalize: aggregate, dedup, calibrate, select placer_py/core/finalize
...
The per-bin work is `placer_py/core/bins.py`, the loop that drives it is
`placer_py/core/scan.py`, the whole-run stage is `placer_py/core/finalize.py`,
and the read filter is `placer_py/io/gate.py`. Each can now be called, tested
and replaced on its own.

THIS IS ALSO THE ONE MODULE IN THE PACKAGE ALLOWED TO IMPORT BOTH SIDES.
`placer_py/core/**` may not import `placer_py.io` -- that rule is what keeps
