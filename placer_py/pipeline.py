"""
The pipeline: BAM in, calls and a ledger out.

Ported from `src/pipeline/pipeline_entrypoints.inc`,
`src/pipeline/pipeline_bin_processing_stage.inc` and the stage wiring in
`src/pipeline/pipeline.cpp`, pinned by `tests/test_32_pipeline.py`.

THE SHAPE OF A RUN:

    stream the BAM in bins
      -> gate1: is this read worth carrying?                 placer_py/reads
      -> cluster signatures into components                  placer_py/clustering
      -> fetch each component's local reads once             placer_py/interval_cache
      -> extract insert fragments                            placer_py/fragments
      -> enumerate breakpoint hypotheses                     placer_py/breakpoints
      -> tally reads per hypothesis, collapse, triage        placer_py/events,
                                                             placer_py/hypotheses
      -> for the shortlist only:
           consensus                                         placer_py/consensus
           segmentation                                      placer_py/segmentation
           TE alignment                                      placer_py/te_classifier
           boundary + joint decision                         placer_py/policy
      -> select one call per component, re-anchor            placer_py/call_selection
    then, once, over the whole run:
      -> finalize: aggregate, dedup, calibrate, select       placer_py/finalization
...
thousands of times more expensive per candidate, so the pipeline's structure is
...
def run_pipeline(reads: Iterable[AlignedRead], chromosome_name: Callable[[int], str],
    ...
    """Stream reads through every stage, then finalize once.

    THE SPLIT IS THE POINT. Everything inside the loop is per-bin and knows
    nothing about the rest of the genome; `finalize_final_calls` runs once and
    is the only stage that can measure the run's own null, control FDR across
    it, or recognise that two bins reported one event. A pipeline that made
    final decisions inside the loop could do none of those.
    """
    result = PipelineResult()
