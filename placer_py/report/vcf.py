...
`placer_py/core/bins.py` sets `FinalCall.strand` to "NA" unconditionally --
this port never resolves insertion orientation, and `placer_py/schema.py` lists
...
def vcf_header_lines(result, context: ReportContext,
    ...
    lines.append(f"##source=placer-py {__version__}")
